...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.data_path.endswith('.h5'):
            ...
            try:
                # Standard HDF5 reader
                df_raw = pd.read_hdf(file_path)
        
            except Exception:
                # fallback for raw h5 format
            ...
            # Dataset_Custom expects date column
        ...
        if 'date' in cols:
            cols.remove('date')
        # forecasting datasets with explicit target
        if self.target in df_raw.columns:
            df_raw = df_raw[['date'] + cols + [self.target]]

        # multivariate datasets like METR-LA
        else:
